...
def test_pairwise_distances(embeddings, squared=False):
...
def test_batch_all_triplet_loss(embeddings, labels, margin=0):
    # 得到每两两embeddings的距离，然后增加一个维度，一维需要得到（batch_size, batch_size, batch_size）大小的3D矩阵
    # 然后再点乘上valid 的 mask即可
    pairwise_distances = test_pairwise_distances(embeddings)
    ...
    # 满足i!=j!=k and i=j,i!=k 的各个组合的平均损失矩阵， 满足i!=j!=k and i=j,i!=k 且损失值大于 1e-16 的占 满足i!=j!=k and i=j,i!=k的的比例
...
def test_anchor_positive_triplet_mask(labels):
    ...
    indices_equal = np.cast[bool](np.eye(np.shape(labels)[0]))
    indices_not_equal = np.logical_not(indices_equal)
...
def test_get_anchor_negative_triplet_mask(labels):
    # 得到negative的2D mask， 只需 [a, n] 对应的 labels 不一致即可
    labels_equal = np.equal(np.expand_dims(labels, 0), np.expand_dims(labels, 1))
    mask = np.logical_not(labels_equal)
    return mask


def test_batch_hard_triplet_loss(embeddings, labels, margin=0):
    # 还是得到两两的距离pairwise_distances
    # 计算最大的positive距离，只需要取每行最大元素即可
    pairwise_distances = test_pairwise_distances(embeddings)
    mask_anchor_positive = test_anchor_positive_triplet_mask(labels)
    mask_anchor_positive = np.cast[float](mask_anchor_positive)
    ...
    mask_anchor_negative = np.cast[float](mask_anchor_negative)
...
if __name__ == '__main__':
    latents = np.random.random((100, 256))
    labels = np.random.randint(0, 5, (100,))
    l1 = test_batch_all_triplet_loss(latents, labels, margin=0)
    l2 = test_batch_hard_triplet_loss(latents, labels, margin=0)
    print()
